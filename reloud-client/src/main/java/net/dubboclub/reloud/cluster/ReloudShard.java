...
package net.dubboclub.reloud.cluster;
...
public class ReloudShard extends Synchronizer implements ReloudNotify{
    ...
    private volatile HashMap<String,JedisPool> slaves = new HashMap<String, JedisPool>();

    private AtomicLong counter = new AtomicLong(0);

    private volatile boolean available=true;

    private volatile String masterAddress=Constants.NONE;
    ...
    /**
     * 从zookeeper中获取最新的当前分片集群节点信息，第一个是master几点，后面的全部认为是slave节点
     * @param children
     */
    public synchronized void notify(final List<String> children) {
        long changeHash = generateChildrenHash(children);
        //本次变更和上次一样，则不需要进入刷新阶段
        if(changeHash==lastChangeHash){
            return ;
        }
        //记录本次刷新hash
        lastChangeHash = changeHash;
        refreshing(new RefreshingHandler() {
            public void  refreshing() {
                if(children.size()==0){
                    ...
                    available=false;
                    if(!masterAddress.equals(children.get(0))){
                        //如果第一个节点和当前master的地址不一样，说明变更了master节点，那么就需要关闭当前的主节点，创建新的节点
                        if(master!=null){
                            master.destroy();
                        }
                        masterAddress = children.get(0);
                        master = createClient(masterAddress);
                    }
                    refreshSlaves(children.subList(1,children.size()));
                }
            }
        });
    }

    private long generateChildrenHash(List<String> children){
        if(children.size()==0){
            return -1;
        }
        ...
        hashKey.append(children.get(0));
        children = children.subList(1,children.size());
        //避免slave节点排序的干扰
        Collections.sort(children);
        for(String child:children){
            hashKey.append(child);
        }
        return HashAlgorithm.KETAMA_HASH.hash(hashKey.toString());
    }
    ...
    private void refreshSlaves(List<String> slaveAddresses){
        ...
        //将更新完只会，某些老的slave节点并不在本次更新的slave列表中，进行销毁
        for(String address:slaves.keySet()){
            if(!newSlaves.containsKey(address)){
                slaves.get(address).destroy();
            }
        }
        //将新的slave节点复制到slave集群变量上
        slaves=newSlaves;
    }

    private JedisPool createClient(String connection){
        String[] items = StringUtils.split(connection, Constants.PORT_SPLIT);
        if(items.length==0){
            JedisPool jedisPool = new JedisPool(jedisPoolConfig,items[0],Integer.parseInt(items[0]));
            return jedisPool;
        }
        return null;
    }
    ...
    public void destroy(){
        ...
        for(JedisPool jedisPool:slaves.values()){
            jedisPool.destroy();
        }
        slaves.clear();
    }
