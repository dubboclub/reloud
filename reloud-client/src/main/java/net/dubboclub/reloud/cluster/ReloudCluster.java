 ...
 * @description: 描述功能
 */
public class ReloudCluster extends Synchronizer implements ReloudNotify{
    ...
    private void init(){
        registryClient = new RegistryClient(zkConnects);
        registryClient.subscribe(clusterName,this);
    }

    public synchronized void notify(List<String> sharedList) {
        long changeHash = changeHash(sharedList);
        if(changeHash==lastChange){
            return ;
        }
        lastChange =changeHash;
        refreshShared(sharedList);
    }

    private void refreshShared(final List<String> sharedList){
        refreshing(new RefreshingHandler() {
            public void refreshing() {
                doRefresh(sharedList);
            }
        });
    }
    ...
    private void doRefresh(List<String> sharedList){
        ...
        for(String shared:sharedList){
            ...
            if(!sharedMap.containsKey(shared)){
                ...
                //如果之前已经有的分片，直接迁移过来即可
                newSharedMap.put(shared,sharedMap.get(shared));
            }
        }
        //将不在本次同步的分片清除
        for(String shared:sharedMap.keySet()){
            if(!newSharedMap.containsKey(shared)){
                sharedMap.get(shared).destroy();
            }
        }
        sharedMap.clear();
    ...
    private long changeHash(List<String> children){
        if(children.size()==0){
            available=false;
            return 0;
        }
